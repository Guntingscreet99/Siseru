                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3" style="display: flex; justify-content: space-between">
                                <div class="form-group">
                                    <a href="{{ url('user/menu/modul/tampil') }}" class="btn btn-primary">
                                        ...
                                        @else
                                            @foreach ($modul as $item)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="{{ asset('storage/' . $item->fileModul) }}"
                                                            target="_blank">
                                                    ...
                                                    {{-- <td>
                                                        <form method="POST"
                                                            action="{{ url('admin/modul/update-status') }}">
                                                            @csrf
                                                            <input type="hidden" name="kdmodul"
                                                                value="{{ $item->kdmodul }}">
                                                    ...
                                                    <td>
                                                        <!-- Button trigger modal -->
                                                        <a href="{{ url('user/modul/ubah/' . $item->kdmodul) }}"
            ...
            $(document).ready(function() {
                ...
                $('#search').on('input', function() {
                    ...
                    timer = setTimeout(function() {
                        $.ajax({
                            url: "{{ url('user/modul-cari') }}", // Gunakan URL absolute
                            method: "GET",
                            data: {
                                query: query
                            },
                            success: function(data) {
                                ...
                                if (data.length === 0) {
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ url('user/modul-edit/') }}/${item.id}" class="btn btn-warning">
                                                <i class="fas fa-pen"></i> Edit
                                            </a>
                                            ...
                                            <div class="modal fade" id="${deleteModalId}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="deleteLabel${item.id}" aria-hidden="true">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        ...
                                                        <form action="{{ url('user/modul-hapus') }}/${item.id}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
